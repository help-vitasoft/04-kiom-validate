    ...
    for(var i=0; i<testArray.length; i++) {
      if(testArray[i].trim() != '') {
        ...
        if(str) {
          ...
          if(vita.getFlag()) vita.test13(editor, str, position, str0);
          if(vita.getFlag()) vita.test14(editor, str, position);
          if(vita.getFlag()) vita.test15(editor, str, position, str0);
          if(vita.getFlag()) vita.test16(editor, str, position);
        }
      }
    }
